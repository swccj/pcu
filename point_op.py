...
def nonuniform_sampling(num = 4096, sample_num = 1024):#非均匀采样
    sample = set()
    loc = np.random.rand()*0.8+0.1#np.random.rand返回一个或一组服从“0~1”均匀分布的随机样本值
    while(len(sample)<sample_num):
        a = int(np.random.normal(loc=loc,scale=0.3)*num)#np.random.normal正态分布，loc为均值，scale正态分布的标准差，对应分布的宽度
        if a<0 or a>=num:
            continue
        sample.add(a)
    return list(sample)
