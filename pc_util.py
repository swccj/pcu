...
def extract_knn_patch(queries, pc, k):
    ...
    pc [P, C]
    """
    knn_search = NearestNeighbors(n_neighbors=k, algorithm='auto')#k近邻搜索
    knn_search.fit(pc)
    knn_idx = knn_search.kneighbors(queries, return_distance=False)#查找点的 K 邻域。返回每个点的索引和到相邻点的距离（return_distance=True时才返回）
    k_patches = np.take(pc, knn_idx, axis=0)  # M, K, C  沿axis轴取数组
    return k_patches
...
class FarthestSampler:
    ...
    def __call__(self, pts, k):
        ...
        id = np.random.randint(len(pts))
        farthest_pts[0] = pts[id]
...
def FarthestPointSampling_ForBatch(xyz, npoint):
    ...
    barycenter = np.sum((point), 1)
    barycenter = barycenter / point.shape[1]
    barycenter = barycenter.reshape(B, 1, 3)

    dist = np.sum((point - barycenter) ** 2, -1)
    ...
    for i in range(npoint):
        ...
        centroid = tf.reshape(point[batch_indices, farthest, :], (B, 1, 3))
        dist = np.sum((point - centroid) ** 2, -1)
        mask = dist < distance
...
def grouping_operation(points, idx):
    ...
    B = points.shape[0]
    for i in range(B):
        point = tf.slice(points, begin=[i, 0, 0], size=[1, tf.shape(points)[1], tf.shape(points)[2]])
        idx_ = tf.slice(idx, begin=[i, 0], size=[1, tf.shape(idx)[1]])
        group = op.index_points(point, idx_)
        if groups is not None:
            groups = tf.concat([groups, group], axis=0)
        else:
            groups = group
    return groups
